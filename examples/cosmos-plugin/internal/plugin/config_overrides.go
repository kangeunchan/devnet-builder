...
func (n *CosmosNetwork) GetConfigOverrides(nodeIndex int, opts network.NodeConfigOptions) ([]byte, []byte, error) {
	...
	appToml, err := buildAppTomlOverrides(n.BaseDenom(), opts)
	if err != nil {
		return nil, nil, err
	}
...
func buildConfigTomlOverrides(moniker string, opts network.NodeConfigOptions) ([]byte, error) {
	data := map[string]any{
		"moniker": moniker,
		"rpc": map[string]any{
			"laddr": tcpListenAddress(listenHostAll, opts.Ports.RPC),
		},
		"p2p": map[string]any{
			"laddr":            tcpListenAddress(listenHostAll, opts.Ports.P2P),
			"persistent_peers": strings.TrimSpace(opts.PersistentPeers),
		},
		"consensus": map[string]any{
			"timeout_commit": consensusTimeoutCommitDevnet,
		},
	}
...
func buildAppTomlOverrides(baseDenom string, opts network.NodeConfigOptions) ([]byte, error) {
	...
	data := map[string]any{
		...
		"api": map[string]any{
			...
			"address":             tcpListenAddress(listenHostAll, opts.Ports.API),
		},
		"grpc": map[string]any{
			"enable":  true,
			"address": listenAddress(listenHostAll, opts.Ports.GRPC),
		},
	}
